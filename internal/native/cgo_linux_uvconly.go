...
func videoSetStreamQualityFactor(factor float64) error {
    ...
    factorC := C.float(factor)
    ret := C.jetkvm_video_set_quality_factor(factorC)
    if ret != 0 {
        return fmt.Errorf("failed to set stream quality factor: %d", ret)
    }
    return nil
}
...
func videoGetEDID() (string, error) {
    ...
    edid := C.jetkvm_video_get_edid_hex()
    if edid == nil {
        return "", fmt.Errorf("failed to get EDID")
    }
    defer C.free(unsafe.Pointer(edid))
