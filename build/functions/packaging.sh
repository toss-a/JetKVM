...
pack_img() {
    ...
    local target_img_name="JetKVM_by-SilentWind_${device_name_friendly}_${DATE}.img"
    ...
    if [ "$device_name_friendly" = "Vm" ]; then
        ...
        local vmdk_img="$OUTPUTDIR/JetKVM_by-SilentWind_Vmare-uefi_${DATE}.vmdk"
        local vdi_img="$OUTPUTDIR/JetKVM_by-SilentWind_Virtualbox-uefi_${DATE}.vdi"
...
pack_img_onecloud() {
    local target_img_name="JetKVM_by-SilentWind_Onecloud_${DATE}.burn.img"
